...
namespace School.Financial.Dac.Impl
{
    public class SchoolDac : ISchoolDac
    {
        ...
        public int Insert(SchoolData data)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("INSERT INTO `School` VALUES (0,@Name,@EducationAreaId,@CreatedDate)", conn);
        ...
        public void Update(SchoolData data)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("UPDATE `School` SET `Name`=@Name,`EducationAreaId`=@EducationAreaId WHERE `Id`=@Id", conn);
                cmd.Parameters.AddWithValue("@Name", data.Name);
                cmd.Parameters.AddWithValue("@EducationAreaId", data.EducationAreaId);
        ...
        public int Upsert(SchoolData data)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("INSERT INTO `School` VALUES (@Id,@Name,@EducationAreaId,@CreatedDate)" +
                    "ON DUPLICATE KEY UPDATE `Name`=@Name,`EducationAreaId`=@EducationAreaId", conn);
                cmd.Parameters.AddWithValue("@Id", data.Id);
                cmd.Parameters.AddWithValue("@Name", data.Name);
                cmd.Parameters.AddWithValue("@EducationAreaId", data.EducationAreaId);
                cmd.Parameters.AddWithValue("@CreatedDate", DateTime.UtcNow);
